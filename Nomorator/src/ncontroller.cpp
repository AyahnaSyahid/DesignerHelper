...
const QSizeF &Controller::pageSize() const { return m_pageSize; }

void Controller::setPageSize(const QSizeF &newPageSize) {
  if (m_pageSize == newPageSize) return;
  m_pageSize = newPageSize;
  nview.setPageSize(m_pageSize);
  emit pageSizeChanged();
}

const QSizeF &Controller::dupOffset() const { return m_dupOffset; }

void Controller::setDupOffset(const QSizeF &newDupOffset) {
  if (m_dupOffset == newDupOffset) return;
  m_dupOffset = newDupOffset;
  recreateTemplate();
  emit dupOffsetChanged();
}

QPointF Controller::startPos() const { return m_startPos; }

void Controller::setStartPos(QPointF newStartPos) {
  if (m_startPos == newStartPos) return;
  m_startPos = newStartPos;
  emit startPosChanged();
  updateView();
}

int Controller::displayPage() const { return m_displayPage; }

void Controller::setDisplayPage(int newDisplayPage) {
  if (m_displayPage == newDisplayPage) return;
...
const QSizeF &Controller::offsets() const { return m_offsets; }

void Controller::setOffsets(const QSizeF &newOffsets) {
  if (m_offsets == newOffsets) return;
...
const QFont &Controller::font() const { return m_font; }
...
void Controller::setStart(const int &val) {
  if (start() != val) {
    ...
    if (totalPage() != cpage) emit parameterChanged("totalPage", totalPage());
  }
  updateView();
}
...
void Controller::setRow(const int &val) {
  if (row() != val) {
    ...
    if (pc != totalPage()) emit parameterChanged("totalPage", totalPage());
    updateView();
  }
}
    ...
    for (int c = 0; c < dim[1]; ++c) {
      ...
      if (c % 2) {
        if (!m_dupOffset.isNull()) {
          ...
          current_item->setPos(m_startPos + QPointF(m_dupOffset.width() * c,
                                                    m_dupOffset.height() * r));
          continue;
        }
      }
      current_item->setText(cpage.data[r][c].c_str());
