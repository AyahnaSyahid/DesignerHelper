...
#include <cstring>
#include <memory>
NomoratorPage::NomoratorPage(int pn, int r, int c,
                             vector<vector<std::string>> st)
...
int Nomorator::totalPage() const {
  ...
  return range % pageDimension() ? (range / pageDimension()) + 1
                                 : range / pageDimension();
}

void Nomorator::setPrefix(const string prf) { extra[0] = prf; }

void Nomorator::setSufix(const string sfx) { extra[1] = sfx; }
...
NomoratorPage Nomorator::getPage(int n) {
  ...
  int incr_row = use_un_bind && (current_bind == full_bind) ? un_bind * column()
                                                            : bind() * column();
  ...
  for (int r = 0; r < row(); ++r) {
    ...
    for (int c = 0; c < column(); ++c) {
      int cn = start() + use_un_bind + (current_bind * incr_bind) +
               (r * incr_row) + (c * incr_col);
      memset(buffer, 0, 100);
      snprintf(buffer, 100, fmt, cn);
      if (padChar() != '0') {
        int nth = 0;
        while (buffer[nth] == '0') buffer[nth++] = padChar();
      }
