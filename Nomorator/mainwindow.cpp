...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent) ,ui(new Ui::MainWindow), m_controller(new Controller(this))
{
    ui->setupUi(this);
    ...
    ui->stext->setText(m_controller->getNomorator()->getPage(0).data[0][0].c_str());
...
{
    connect(m_controller, &Controller::parameterChanged, this, &MainWindow::interpretNomoratorChanges);
    connect(ui->nstart, SIGNAL(valueChanged(int)), m_controller, SLOT(setStart(int)));
    connect(ui->nstop, SIGNAL(valueChanged(int)), m_controller, SLOT(setStop(int)));
    connect(ui->bd, SIGNAL(valueChanged(int)), m_controller, SLOT(setBind(int)));
    connect(ui->col, SIGNAL(valueChanged(int)), m_controller, SLOT(setColumn(int)));
    connect(ui->row, SIGNAL(valueChanged(int)), m_controller, SLOT(setRow(int)));
    connect(ui->cpad, SIGNAL(textChanged(QString)), m_controller, SLOT(setPad(QString)));
    connect(ui->npad, SIGNAL(valueChanged(int)), m_controller, SLOT(setPad(int)));
    connect(ui->prf, SIGNAL(textChanged(QString)), m_controller, SLOT(setPrefix(QString)));
    connect(ui->sfx, SIGNAL(textChanged(QString)), m_controller, SLOT(setSufix(QString)));
    connect(ui->pwidth, SIGNAL(valueChanged(double)), this, SLOT(pageSizeUpdate()));
    connect(ui->pheight, SIGNAL(valueChanged(double)), this, SLOT(pageSizeUpdate()));
    connect(ui->nposx, SIGNAL(valueChanged(double)), this, SLOT(startPosUpdate()));
    connect(ui->nposy, SIGNAL(valueChanged(double)), this, SLOT(startPosUpdate()));
    connect(ui->dupx, SIGNAL(valueChanged(double)), this, SLOT(duplicationUpdate()));
    connect(ui->dupy, SIGNAL(valueChanged(double)), this, SLOT(duplicationUpdate()));
    connect(ui->nofx, SIGNAL(valueChanged(double)), this, SLOT(offsetSizeUpdate()));
    connect(ui->nofy, SIGNAL(valueChanged(double)), this, SLOT(offsetSizeUpdate()));
    connect(ui->pdup, &QRadioButton::toggled, this, [=](bool tg) {if(!tg) {ui->dupx->setValue(0);ui->dupy->setValue(0);}});
    connect(m_controller->getView(), &NomoratorView::startPosMoved,
            this,
            [=](QPointF f){
            blockSignals(true);
...
void MainWindow::interpretNomoratorChanges(const QString &key, const QVariant &var)
{
    qDebug() << key << " changed (" << var << ")" ;
    if(key == "prefixStr" || key == "suffixStr" || key == "padChar" || key == "padCount")
        ui->stext->setText(m_controller->getNomorator()->getPage(0).data[0][0].c_str());
}
...
{
    auto fn = QFileDialog::getOpenFileName(this, "Pilih Background(Master)", QDir::homePath(), "Image File ( *.jpg *.png *.jpeg)");
    if(!fn.isEmpty()) {
