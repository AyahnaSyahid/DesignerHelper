...
namespace PolaroidMakerNS {
    struct SizeFLists {
        ...
        SizeFLists(const QString& j="Custom", const QString& n = "Custom", double w = 0, double h = 0) : jenis(j), nama(n), ukuran(w, h) {}
        ~SizeFLists(){};
    };
    ...
    public:
            ...
            if(availableJenis().contains(jenis)) {
                if(availableNama().contains(nama)) {
                    for(auto sz = begin(); sz < end(); ++sz) {
                        if(sz->jenis == jenis && sz->nama == nama)
                            return sz->ukuran;
                    }
                }
            }
        ...
        void saveToFile(const QString &fn)
        {
            ...
            QString cf;
            for (auto j = begin(); j != end(); ++j) {
                cf = fmt.arg(j->jenis, j->nama).arg(j->ukuran.width()).arg(j->ukuran.height());
        ...
        void loadFromFile(const QString &fn)
        {
            ...
            while(!txs.atEnd()) {
                ...
                try {
                    SizeFLists got(stlist[0], stlist[1], stlist[2].toDouble(), stlist[3].toDouble());
                    append(got);
                }  catch (...) {
                    continue;
                }
