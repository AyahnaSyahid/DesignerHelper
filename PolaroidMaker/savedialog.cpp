...
SaveDialog::SaveDialog(QWidget *parent, PolaroidMaker *mr)
    : QDialog(parent), maker(mr), ui(new Ui::SaveDialog) {
  ui->setupUi(this);
...
void SaveDialog::on_openSave_clicked() {
  auto selName =
      QDateTime::currentDateTime().toString("yyyy-MM-dd HH_mm_ss") + ".pdf";
  auto fn = QFileDialog::getSaveFileName(this, "Simpan sebagai", selName,
                                         "PDF (*.pdf)");
  if (fn.isEmpty())
    QMessageBox::warning(this, "Dibatalkan",
                         "Anda harus memilih tempat untuk menyimpan file");
  else {
