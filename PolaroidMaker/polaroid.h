...
class POLAROIDMAKER_EXPORT Polaroid {
 public:
  enum ScaleMode { Blured, Expanded };
 ...
 public:
  ...
  Polaroid(const QString &image = QString(), const QSizeF &size = {63.6, 95},
           const QPointF &position = {0, 0}, bool e_cropmark = true,
           qreal cropmark_size = 2, qreal cropmark_weight = 0.1,
  ...
  // ~Polaroid();
  QImage getImage();
  inline QString imageName() noexcept {
    return m_image.isEmpty() ? "" : QFileInfo(m_image).baseName();
  }
