...
void Polaroid::applyGamma(QImage *tg, const float& gamma) {
    ...
    if(!ctable.isEmpty()) {
        for(auto &color : ctable) {
            ...
            auto col = qRgba(255 * pow(r, gamma),
                               255 * pow(g, gamma),
                               255 * pow(b, gamma),
                               255 * pow(a, gamma));
            color = col;
...
Polaroid::Polaroid(const QString &image,
                   const QSizeF &size,
                   const QPointF& position,
                   bool e_cropmark,
                   qreal cropmark_size,
                   qreal cropmark_weight,
                   const QColor &cropmark_color,
...
void Polaroid::render(QPainter *ptr, const QPointF &pos)
{
    ...
    if(!imf.isNull()) {
        ...
        if(m_scale == Expanded) {
            ...
            full.setY(pixi.boundingRect().center().y() - full.boundingRect().center().y());
            auto srcRect = cRect;
