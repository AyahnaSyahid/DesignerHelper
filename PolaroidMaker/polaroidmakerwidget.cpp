...
PolaroidMakerWidget::PolaroidMakerWidget(QWidget *parent)
  ...
  connect(polModel, &QAbstractItemModel::dataChanged, this,
          &PolaroidMakerWidget::refreshCounter);
  connect(polModel, &QAbstractItemModel::modelReset, this,
          &PolaroidMakerWidget::refreshCounter);
  connect(polModel, &QAbstractItemModel::rowsRemoved, this,
          &PolaroidMakerWidget::refreshCounter);
  connect(polModel, &QAbstractItemModel::rowsInserted, this,
          &PolaroidMakerWidget::refreshCounter);
  ...
  connect(ui->listView->selectionModel(),
          ...
          &PolaroidMakerWidget::deleteToggler);
  connect(this, &PolaroidMakerWidget::ukuranKertasChanged, this, [=]() {
    this->setUkuranKertas(QSizeF(ui->kWSize->value(), ui->kHSize->value()));
  });
  connect(this, &PolaroidMakerWidget::ukuranPolaroidChanged, this, [=]() {
    this->setUkuranPolaroid(QSizeF(ui->pWSize->value(), ui->pHSize->value()));
  });
  ...
  connect(act2, &QAction::triggered, this,
          &PolaroidMakerWidget::openRefillDialog);
  connect(ctr, &Counter::refillSuccess, this,
...
void PolaroidMakerWidget::on_colTButton_clicked() {
  ...
  if (nc.isValid()) {
    if (nc != markColor) {
      ui->colLPrev->setStyleSheet(
          QString("background-color: %1").arg(nc.name(QColor::HexRgb)));
      markColor = nc;
    }
  }
}
...
void PolaroidMakerWidget::on_addFile_clicked() {
  auto fn = QFileDialog::getOpenFileNames(this, "Pilih beberapa gambar", "",
...
void PolaroidMakerWidget::on_eGamma_clicked() {
  ...
  Polaroid &pl = static_cast<PolaroidListModel *>(polModel)->getPolaroid(
      ui->listView->selectionModel()->selectedRows()[0].row());
...
void PolaroidMakerWidget::on_doButton_clicked() {
  ...
  if (!ctr->canAcceptRequest()) {
    ...
    tokenHabis.setText(
        "Silahkan isi token anda agar dapat menggunakan aplikasi kembali");
...
void PolaroidMakerWidget::refillSuccessHandler(int av, int bo) {
  ...
  QMessageBox::information(
      this, "Pengisian Token Berhasil",
      message.arg(QString::number(av), QString::number(bo)));
  lihatCounter();
}
