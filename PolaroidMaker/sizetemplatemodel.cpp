...
SizeTemplateModel::SizeTemplateModel(QObject *parent, PolaroidMakerNS::SizeFListsTable *ls)
    : QAbstractTableModel(parent), s_table(ls)
{
    colHeader << "Jenis" << "Nama" << "Lebar" << "Tinggi";
}
...
QVariant SizeTemplateModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal) {
        if(section >= colHeader.count())
            return QVariant();
...
bool SizeTemplateModel::insertRows(int row, int count, const QModelIndex &parent)
{
...
bool SizeTemplateModel::removeRows(int row, int count, const QModelIndex &parent)
{
...
bool SizeTemplateModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if(!index.isValid()) {
        return false;
    }
