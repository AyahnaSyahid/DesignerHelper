...
GammaEditorDialog::GammaEditorDialog(QWidget *parent, Polaroid *pol)
    : QDialog(parent), mPol(pol), ui(new Ui::GammaEditorDialog) {
  ui->setupUi(this);
  ui->imAft->setFixedSize(QSize(201, 302));
  ui->imBef->setFixedSize(QSize(201, 302));
  before = mPol->getImage().scaled(ui->imBef->width(), ui->imBef->height(),
                                   Qt::KeepAspectRatio);
  after = before;
...
void GammaEditorDialog::setInitialGamma(double g) { initGamma = g; }

GammaEditorDialog::~GammaEditorDialog() { delete ui; }
...
void GammaEditorDialog::on_gslide_valueChanged(int position) {
  ...
  if (position != initGamma * 1000) {
    if (GME::gthread) {
      ...
      GammaWorker *gw = new GammaWorker(
          nullptr, &after,
          (ui->gslide->maximum() - ui->gslide->value()) / 1000.0);
      ...
      connect(GME::gthread, &QThread::finished, GME::gthread,
              &QThread::deleteLater);
...
void GammaEditorDialog::updateToolTiptext() {
  ...
  ui->gslide->setToolTip(
      QString("%1").arg((ui->gslide->maximum() - val) / 1000.0));
}
