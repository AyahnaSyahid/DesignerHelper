...
void PolaroidMaker::notifyUpdate() {
  if (this->updateCB) updateCB();
}

void PolaroidMaker::progressUpdate(const int &page, const int &polIndex) {
  if (progressUpdateCB) progressUpdateCB(page, polIndex);
}

void PolaroidMaker::setUpdateCB(void (*ucb)()) { updateCB = ucb; }

void PolaroidMaker::setProgressUpdateCB(void (*pucb)(const int &,
                                                     const int &)) {
  progressUpdateCB = pucb;
}

void PolaroidMaker::createConnection(QProgressBar *polp, QProgressBar *pagep) {
  updater->connect(updater, &Updater::polaroidUpdate, polp,
                   &QProgressBar::setValue);
  updater->connect(updater, &Updater::pageUpdate, pagep,
                   &QProgressBar::setValue);
}

PolaroidMaker::PolaroidMaker()
    : polModel(new PolaroidListModel()),
      updater(new Updater),
...
int PolaroidMaker::pageToCreate() {
  if (!polModel->rowCount()) return 0;
  auto ld = getLayoutDimension();
  return qCeil(1.0 / (ld.first * ld.second) * polModel->rowCount());
}
...
void PolaroidMaker::addPolaroid(const QString &pic) {
  polModel->insertImages({pic});
  notifyUpdate();
}

void PolaroidMaker::removePolaroid(const int &ind) {
  if (!polModel->rowCount()) return;
  if (ind >= 0 && ind < polModel->rowCount()) {
    polModel->removeRows(ind, 1);
    notifyUpdate();
  }
}
...
bool PolaroidMaker::writeToFile(const QString &fname) {
  ...
  if (ly.first * ly.second < 1) {
    // Calculation Error
    m_failReason =
        QString("Invalid Layout %1 x %2").arg(ly.first).arg(ly.second);
    return false;
  }
  QPdfWriter wrt(fname);
  wrt.setPageSize(QPageSize(
      pageSize, QPageSize::Millimeter,
      QString("Custom %1x%2").arg(pageSize.width()).arg(pageSize.height()),
  ...
  QPainter ptr(&wrt);
  QSizeF drawSize(ly.first * p_polsize.width(), ly.second * p_polsize.height());

  // QPointF fpost((wrt.pageSizeMM().width() - drawSize.width()) / 2,
  // (wrt.pageSizeMM().height() - drawSize.height()) / 2);

  QPointF fpost((wrt.pageLayout().pageSize().definitionSize().width() -
                 drawSize.width()) /
                    2,
                (wrt.pageLayout().pageSize().definitionSize().height() -
                 drawSize.height()) /
                    2);
  ...
  while (itr != p_pols.end()) {
    ...
    {  // Marking Section
      ...
      if (pageNumbering) {
        ...
        auto tw = fnm.boundingRect(pageNumberRect.toRect(),
                                   Qt::AlignVCenter | Qt::AlignHCenter, pn);
      ...
      if (pageMarking) {
        auto fnm = ptr.fontMetrics();
        auto tw = fnm.boundingRect(pageMarkRect.toRect(),
...
QAbstractItemModel *PolaroidMaker::polaroidModel() { return polModel; }
